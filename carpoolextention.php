    ...
    public function registerDriverRoute(Request $request): JsonResponse
        {
           ...
           // }
          $routeData = $this->getRoutePointsFromAPI(
        $request->start_lat,  
        $request->start_lng,  
        $request->end_lat,    
        $request->end_lng     
      ...
      //dina gded
    if (!$routeData || !isset($routeData['points'], $routeData['encoded_polyline'])) {
        return response()->json(responseFormatter(DEFAULT_400, ['message' => 'Unable to retrieve route']), 400);
    }
    ...
    return [
        ...
        return [
          ...
          'allow_luggage' => (bool) $route->allow_luggage,
           ...
           'encoded_polyline' => $route->encoded_polyline, //dina gded
