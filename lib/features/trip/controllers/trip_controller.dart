...
class TripController extends GetxController implements GetxService {
  ...
  /// Kept for backward-compatibility with tripe_screen.dart (old screen).
  void setStatusIndex(int index) {}

  /// Kept for backward-compatibility with app_bar_widget.dart.
  ...
  // ── Carpool trip list ──────────────────────────────────────────────────────
  Future<void> getCarpoolTripList(
    int offset, {
    bool reload = false,
    String status = 'all',
  }) async {
    ...
    if (response.statusCode == 200 && response.body['data'] != null) {
      if (offset == 1) {
        ...
        carpoolTripModel?.totalSize = TripModel.fromJson(response.body).totalSize;
      }
    } else {
      ApiChecker.checkApi(response);
    }
  ...
  // ── Parcel trip list ───────────────────────────────────────────────────────
  Future<void> getParcelTripList(
    int offset, {
    bool reload = false,
    String status = 'all',
  }) async {
    if (reload) {
      parcelTripModel = null;
      update();
    }
    final response = await tripServiceInterface.getTripList(
      'parcel',
    ...
    if (response.statusCode == 200 && response.body['data'] != null) {
      if (offset == 1) {
        parcelTripModel = TripModel.fromJson(response.body);
      } else {
        parcelTripModel?.data
            ?.addAll(TripModel.fromJson(response.body).data ?? []);
        parcelTripModel?.offset    = TripModel.fromJson(response.body).offset;
        parcelTripModel?.totalSize = TripModel.fromJson(response.body).totalSize;
      }
    } else {
      ApiChecker.checkApi(response);
    }
  ...
  void getRideCancellationReasonList() async {
    final response = await tripServiceInterface.getRideCancellationReasonList();
  ...
  void getParcelCancellationReasonList() async {
    final response = await tripServiceInterface.getParcelCancellationReasonList();
    if (response.statusCode == 200) {
